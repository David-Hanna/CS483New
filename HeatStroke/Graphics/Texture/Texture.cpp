...
namespace HeatStroke
{
	...
	{
		if (p_strFile.find(".tga") != std::string::npos)
		{
	...
	Texture::Texture(void* p_pData, unsigned int p_uiWidth, unsigned int p_uiHeight, Format p_eFormat) : 
		m_eFilterMin(Texture::FM_Invalid), 
		m_eFilterMag(Texture::FM_Invalid),
		m_eWrapU(Texture::WM_Invalid), 
		m_eWrapV(Texture::WM_Invalid),
		m_uiWidth(0), 
		m_uiHeight(0), 
		m_uiTex(0)
	{
	...
	void Texture::LoadFromTGA(const std::string& p_strFile)
	{
		...
		m_uiWidth = FreeImage_GetWidth(image);
		m_uiHeight = FreeImage_GetHeight(image);
