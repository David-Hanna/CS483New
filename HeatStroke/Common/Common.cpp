...
void GL_CHECK_ERROR(const char* file, int line)
{
	...
	if (e != GL_NO_ERROR)
	{
		std::cout << "GL ERROR: " << e << ", File: " << file << ", Line: " << line << std::endl;
...
GLuint HeatStroke::Common::CreateTextureFromDDS(const std::string& p_strFile, unsigned int* p_pWidth, unsigned int* p_pHeight, bool* p_pHasMips)
{
	...
	pCurrent += 4;  //skip over header 

	//read the dds header data
	DDS_HEADER* ddsh;

	ddsh = const_cast<DDS_HEADER*>(reinterpret_cast<const DDS_HEADER*>(pCurrent));
	pCurrent += sizeof(DDS_HEADER);
